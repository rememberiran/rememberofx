...
public class FolderService : IFolderService
{
    ...
    public async Task<Result> AddTweetAsync(Guid folderId, Guid tweetId, CancellationToken ct)
    {
        ...
        if (string.Equals(folder.Visibility, "private", StringComparison.Ordinal)
            && folder.CreatedByUserId != userId.Value)
        {
        ...
        var hasRole = identity.Roles.Count > 0;
        var status = hasRole ? "approved" : "pending";
